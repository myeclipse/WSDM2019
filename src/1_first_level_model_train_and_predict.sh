...
#describe
#训练第一层模型，基于第一层模型的输出概率作为第二层模型的输入

#2 模型训练，共计25个基于bert的模型
...
export OUTPUT_DIR=../data/model/output05
python2 -u run_classifier.py \
  ...
  --init_checkpoint=$ZH_BERT_BASE_DIR/bert_model.ckpt \
  --max_seq_length=128 \
  --train_batch_size=32 \
  --learning_rate=5e-5 \
...
python2 -u run_classifier.py \
  ...
  --init_checkpoint=$EN_BERT_BASE_DIR/bert_model.ckpt \
  --max_seq_length=128 \
  --train_batch_size=16 \
  --learning_rate=1e-5 \
...
python2 -u run_classifier.py \
  ...
  --init_checkpoint=$EN_BERT_BASE_DIR2/bert_model.ckpt \
  --max_seq_length=128 \
  --train_batch_size=16 \
  --learning_rate=1e-5 \
  --num_train_epochs=3.0 \
